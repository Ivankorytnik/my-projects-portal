...
const TEMPLATE_IMAGE_WIDTH = 1819;
const TEMPLATE_IMAGE_HEIGHT = 2573;
...
const state = { articles: [], templateManifest: {}, selectedArticle: null, busy: false, templateBusy: false, savedManagers: [], customTemplate: null };
const el = {};
...
function cacheElements() {
  const ids = ['purpose','modification','bodyColor','interiorColor','tint','article','templateInfo','articleDecode','client','basePrice','specialPrice','modelYear','deliveryDate','validTo','manager','phone','email','managerProfile','deleteManagerProfile','managerSaveInfo','templateUpload','resetTemplate','templateUploadInfo','generateButton','progressArea','progressBar','progressText','message'];
...
function attachEvents() {
  ...
  el.templateUpload.addEventListener('change', handleTemplateUpload);
  el.resetTemplate.addEventListener('click', resetCustomTemplate);
}
...
function isImageFile(file) {
  return file && (file.type === 'image/jpeg' || file.type === 'image/png' || /\.(jpe?g|png)$/i.test(file.name));
}

function updateTemplateUi() {
  if (!el.templateUploadInfo || !el.resetTemplate) return;
  const custom = state.customTemplate;
  if (state.templateBusy) {
    el.templateUploadInfo.textContent = 'Обрабатываю загруженный шаблон...';
  } else if (custom) {
    el.templateUploadInfo.textContent = `Загружен шаблон: ${custom.name}. Используется только в этой вкладке браузера.`;
  } else {
    el.templateUploadInfo.textContent = 'Используется стандартный шаблон АТОМ для выбранного цвета.';
  }
  el.resetTemplate.disabled = state.busy || state.templateBusy || !custom;
  if (el.templateUpload) el.templateUpload.disabled = state.busy || state.templateBusy;
  if (state.selectedArticle && el.templateInfo) {
    el.templateInfo.textContent = custom
      ? `Будет использован загруженный шаблон: ${custom.name}`
      : `Будет сформировано полное КП PDF: ${state.selectedArticle.bodyColorDisplay}`;
  }
}

function setTemplateBusy(value) {
  state.templateBusy = value;
  syncGenerateButton();
  updateTemplateUi();
}
...
async function handleTemplateUpload(event) {
  ...
  const previousTemplate = state.customTemplate;
  try {
    ...
    if (files.length === 1 && isPdfFile(files[0])) {
      name = files[0].name;
      pages = await renderUploadedPdf(files[0]);
    } else if (files.length === PAGE_COUNT && files.every(isImageFile)) {
      const sorted = [...files].sort((a, b) => a.name.localeCompare(b.name, 'ru', { numeric: true, sensitivity: 'base' }));
      name = `9 изображений: ${sorted[0].name} ... ${sorted[sorted.length - 1].name}`;
      pages = [];
      for (let i = 0; i < sorted.length; i += 1) {
        el.templateUploadInfo.textContent = `Подготовка страницы ${i + 1} из ${PAGE_COUNT}...`;
        pages.push(await normaliseTemplateImage(sorted[i]));
      }
    } else {
      throw new Error('Выберите один PDF на 9 страниц или ровно 9 файлов JPG/PNG.');
    }

    if (!Array.isArray(pages) || pages.length !== PAGE_COUNT) {
      throw new Error(`В шаблоне должно быть ровно ${PAGE_COUNT} страниц.`);
    }
    state.customTemplate = { name, pages };
    setMessage(`Шаблон загружен: ${name}`, 'success');
  } catch (error) {
    console.error(error);
    state.customTemplate = previousTemplate;
    setMessage(error.message || 'Не удалось загрузить шаблон.', 'error');
  } finally {
    event.target.value = '';
...
function resetCustomTemplate() {
  ...
  if (el.templateUpload) el.templateUpload.value = '';
  updateTemplateUi();
  updateSelectedArticle();
  setMessage('Возвращён стандартный шаблон АТОМ.', 'success');
}
...
async function loadPdfJs() {
  ...
  pdfJsPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      if (!window.pdfjsLib) {
        reject(new Error('Библиотека чтения PDF не загрузилась.'));
        return;
      }
      window.pdfjsLib.GlobalWorkerOptions.workerSrc = `https://cdn.jsdelivr.net/npm/pdfjs-dist@${PDFJS_VERSION}/build/pdf.worker.min.js`;
      resolve(window.pdfjsLib);
    };
    script.onerror = () => reject(new Error('Не удалось загрузить модуль чтения PDF. Можно загрузить шаблон как 9 JPG/PNG.'));
...
async function renderUploadedPdf(file) {
  ...
  try {
    if (pdf.numPages !== PAGE_COUNT) {
      throw new Error(`В PDF должно быть ровно ${PAGE_COUNT} страниц. Сейчас: ${pdf.numPages}.`);
    }
    const pages = [];
    for (let pageNo = 1; pageNo <= PAGE_COUNT; pageNo += 1) {
      el.templateUploadInfo.textContent = `Подготовка страницы ${pageNo} из ${PAGE_COUNT}...`;
      ...
      const scaleX = TEMPLATE_IMAGE_WIDTH / viewport.width;
      const scaleY = TEMPLATE_IMAGE_HEIGHT / viewport.height;
      await page.render({
        canvasContext: ctx,
        viewport,
...
async function normaliseTemplateImage(file) {
...
async function generatePdf() {
  ...
  try {
    ...
    for (let pageNo = 1; pageNo <= PAGE_COUNT; pageNo += 1) {
      ...
      const customPage = state.customTemplate && state.customTemplate.pages
        ? state.customTemplate.pages[pageNo - 1]
        : null;
      if (customPage) {
        if ([1,8,9].includes(pageNo)) jpegPages.push(await renderDynamicPage(customPage, pageNo, row, input));
        else jpegPages.push(customPage.slice());
      } else {
...
async function renderDynamicPage(source, pageNo, row, input) {
  let blob;
  if (source instanceof Uint8Array) {
    blob = new Blob([source], { type: 'image/jpeg' });
  } else {
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Не удалось загрузить страницу шаблона: ${source}`);
...
function syncGenerateButton(){ el.generateButton.disabled=state.busy||state.templateBusy||!state.selectedArticle; }
function setBusy(value){ state.busy=value; syncGenerateButton(); el.generateButton.textContent=value?'Формирование PDF...':'Сформировать и скачать PDF'; el.progressArea.hidden=!value&&Number(el.progressBar.style.width.replace('%',''))===0; updateTemplateUi(); }
